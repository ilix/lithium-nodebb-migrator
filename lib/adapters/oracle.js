...
function getLithiumAccounts (count) {
  console.log('oracle.getLithiumAccounts()')
  const sql = `
    SELECT   LITHIUM_ACCOUNT.ID
            ...
            ,LITHIUM_ACCOUNT.SUBSCRIPTION_ID
    FROM LITHIUM_ACCOUNT
    LEFT OUTER JOIN SUBSCRIPTION ON LITHIUM_ACCOUNT.SUBSCRIPTION_ID = SUBSCRIPTION.ID
    WHERE rownum < :count`
  return execute([conn => conn.execute(sql, [count])])
    .then(res => res)
}
