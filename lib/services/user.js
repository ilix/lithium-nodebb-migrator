...
function saveUserObject (userObject) {
  ...
  return api.saveUser(userObject)
    ...
    .then(legacyUser => {
      if (!legacyUser.id) {
        failures++
        logger.error(`Could not find user ${userObject.email} / ${userObject.legacyId} / ${userObject.username}`)
        return
      }
      ...
      const updateUser = {
        lastonline: legacyUser.last_visit_time
      }

      if (legacyUser.signature) {
        updateUser.signature = legacyUser.signature
      }

      if (legacyUser.picture) {
        let p = legacyUser.picture
        p = p.replace(p.substring(0, p.indexOf('image-id') + 9), '')
        p = p.replace(p.substring(p.indexOf('/image-size/')), '')

        p = `${process.env.IMAGE_SERVER}/image/resized/${p}?size=medium`

        updateUser.picture = p
        updateUser.uploadedpicture = p
      }

      return mongo.upsert(updateUser, {_key: `user:${uid}`})
    })
    .then(() => {
      let ssoName_uid = {}
