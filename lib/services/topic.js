...
function save (message, categoryId, nodeId) {
  ...
  return api.saveTopic(topic)
    .then(saveResult => {
      ...
      const data = {
        uid,
        pid: saveResult.payload.topicData.mainPid,
...
module.exports = {
  ...
  processBlog: () => {
    console.log('jaha')
    return importTopicsForNode(process.env.BLOG_NODE_ID, process.env.BLOG_CATEGORY_ID)
      .then(() => {
        logger.info('Imported blog posts')
      })
      .catch(error => {
        logger.error(error)
      })
