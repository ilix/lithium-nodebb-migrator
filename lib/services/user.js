...
function saveUserObject (userObject) {
  ...
  return api.saveUser(userObject)
    ...
    .then(legacyUser => {
      if (!legacyUser.id) {
        failures++
        logger.error(`Could not find user by email: ${userObject.email}`)
        return
      } else {
    ...
    .catch(error => {
      failures++
      logger.error(`Failed to save user ${userObject.email} in api: ${error.statusMessage}. Perhaps the user is already imported?`)
    })
}
