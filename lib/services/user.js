...
function findUser ({email, ssoId, username}) {
  return mysql.getUserByEmail(email)
  .then(user => {
    if (!user) {
      logger.debug(`Could not find user by: ${email}`)
      return mysql.getUserBySSOId(ssoId)
    }
    return Promise.resolve(user)
  })
  .then(user => {
    if (!user) {
      logger.debug(`Could not find user by: ${ssoId}`)
...
function saveUserObject (userObject) {
  ...
  return api.saveUser(userObject)
    ...
    .then(legacyUser => {
      ...
      if (!legacyUser.id) {
        failures++
        logger.error(`Could not find user ${userObject.email} / ${userObject.sso_id}`)
        return
      } else {
    ...
    .then(() => {
      ...
      if (userObject['vimla-nodebbId'] != null) {
        ...
        return mongo.upsert(ssoName_uid, JSON.parse(JSON.stringify(ssoName_uid)))
      } else {
